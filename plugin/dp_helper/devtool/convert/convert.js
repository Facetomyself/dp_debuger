...
var extract_code_ele_xpath = '(function() { \
var elem = $0; \
if (elem) {return XPathParser.getXPath(elem) \
} \
})()';

window.sao_initial_code=`
...
window.sao_search_history=["暂存测试"];
...
function outerHTMLToJSON(outerHTML) {
    ...
    if (element && element != null && element != undefined && element.tagName!=undefined) {
...
function main_job() {
    ...
    chrome.devtools.inspectedWindow.eval(
     extract_code_ele_xpath ,
     (result, isException) => {
         if (isException) {
             console.error('Error:', isException); 
         }  else {
             console.log('返回当前元素xpath结果:', result);
             window.xpath_info = result;
             document.getElementById('xpath_info2').value= result; 
...
function updateSelectedCheckboxes() {
  ...
  document.getElementById('xuanze_info').value=xuanze_info;
...
copyBTN_css.addEventListener('click', () => {
  copyToClipboard('css_content', copyBTN_css, '成功', '复制');
});
